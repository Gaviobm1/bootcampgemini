  ...
  return (control: AbstractControl): Record<string, any> | null => {
    ...
    const err = {
      nifnie: {
        invalidFormat: true,
        invalidChar: true,
        message: 'NIF o NIE invalido',
      },
    };
    if (
      /^((\d{1,8})|([X-Z]\d{7}))[TRWAGMYFPDXBNJZSQVHLCKE]$/.test(
...
@Directive({
  ...
  providers: [
    { provide: NG_VALIDATORS, useExisting: NIFNIEValidator, multi: true },
  ],
})
...
@Directive({
  selector:
    '[uppercase][formControlName],[uppercase][formControl],[uppercase][ngModel]',
  providers: [
...
export function lowercaseValidator(): ValidatorFn {
  return (control: AbstractControl): Record<string, any> | null => {
    ...
    return control.value === control.value.toLowercase()
      ? null
      : { lowercase: 'Tiene que estar en mayúsculas' };
  };
}
@Directive({
  selector:
    '[lowercase][formControlName],[lowercase][formControl],[lowercase][ngModel]',
  providers: [
    { provide: NG_VALIDATORS, useExisting: LowercaseValidator, multi: true },
  ],
})
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => TypeValidator),
      multi: true,
    },
  ],
})
...
export function ibanValidator(
  ...
  return isIBAN(control.value.toString())
    ? null
    : { iban: 'No es un IBAN valido' };
}
@Directive({
...
export function isNotBlankValidator(
  ...
  return control.value != null &&
    ...
    ? null
    : { isNotBlank: 'No puede estar vacío' };
}
